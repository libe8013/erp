...
layui.use(['jquery','form','layer','table'],function () {
    ...
    var active = {
        addRow : function () {//添加行方法
            ...
            var newData = {id:uuid,uuid:null,inprice:null,num:null,money:null,inventory:null};
        ...
        saveOrders: function(){
            ...
            if(supplieruuid==0 || supplieruuid==null || supplieruuid==""){
    ...
    table.on('edit(orderDetailTab)',function (obj) {
        ...
        var b = false;
        $.ajax({
            url : path+'/store/queryStoreGoodsSupplier',
            data : {goodsuuid : data.uuid,supplierid:supplierUUID},
            dataType : 'json',
            type : 'post',
            async : false,
            success : function (data) {
...
function initTable(tableId){
    return layui.table.render({
        ...
        cols: [[ //表头
            ...
            {field:'inventory',width:'15%',title:'库存',align:'center',templet:function (row) {
                if(GoodsResult.length==0){
                    return '';
                }else{
                    for(var i=0;i<GoodsResult.length;i++){
                        if(row.uuid==GoodsResult[i].uuid){
                            return GoodsResult[i].inventory;
                        }
                    }
                }
                return '';
...
function getGoods(supplierUUID){
    var result = [];
    $.ajax({
        url : path+'/orders/querySupplierGoods',
