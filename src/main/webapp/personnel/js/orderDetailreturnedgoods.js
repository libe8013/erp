...
layui.use(['jquery','table','layer'],function () {
    ...
    $('#Ordersreturnorders').click(function () {
        layer.confirm('确定执行登记退货订单操作嘛?', function (index) {
            var uuid = $('#uuid').val();
            var creater = $('#createrUUID').val();
            ReturnedGoods(uuid, creater);
        });
    });
...
function ReturnedGoods(uuid,creater){
    $.ajax({
        url : path+'/returnedorders/ReturnedGoods',
