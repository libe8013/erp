...
function initTable(){
    ...
    return table.render({
        ...
        cols: [[ //表头
            ...
            {field:'操作', width:'10%', title: '操作',align:'center',templet:function(obj){
                if(obj.state=="已审核"){
                    return '<a class="layui-btn layui-btn-normal layui-btn-sm" id="orderdetailAudit" lay-event="queryOrderDetailAudit">订单确认</a>';
                }else {
                    return '';
                }
            }}
...
function queryOrders() {
    var url = path+'/orders/queryPurchasePager?1=1&state=已审核&type=采购';

    var starttime = $('#createtime').val();

    if(''!=starttime && null!=starttime){
        url+="&createtime="+starttime;
    }
...
function querySingleOrderDetail(data,tr){
    layer.open({
        ...
        area : ['1030px' , '600px'],
        content: path+"/purchase/jsp/orderDetailAffirm.jsp?uuid="+data.uuid,
