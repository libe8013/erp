...
function queryOrdersStorage() {
    var url = path+'/returnedorders/queryReturnOrdersPager?1=1&state=已审核&type=销售';

    var creatertime = $('#creatertime').val();

    if(''!=creatertime && null!=creatertime){
        url+="&creatertime="+creatertime;
    }
...
function querySingleOrderDetail(data,tr){
    layer.open({
        ...
        area : ['1030px' , '660px'],
        content: path+"/market/jsp/returnorderDetailStorage.jsp?uuid="+data.uuid,
